...
{viewerjumpto "Remarks" "clspl2##remarks"}{...}
{viewerjumpto "Examples" "clspl2##examples"}{...}
{viewerjumpto "Stored results" "clspl2##results"}{...}

{title:Title}
{phang}
{cmd:clspl2} {hline 2} Convex Least Squares Programming (CLSP) is a modular
two-step estimator for solving underdetermined, ill-posed, or structurally
constrained least-squares problems, based on the L2 norm (i.e., implementing the
{bf:alpha} = {bf:1} special case)
...
{syntab:Other}
{synopt :{opt condtol:erance}}Singular-value cutoff for the custom condition
        number function. If set to a {bf:missing value}, the implementation
        uses an internal relative cutoff of {bf:1e-14}.
...
{break}For a Python-based Stata alternative, consult {helpb clsp}.
...
{title:Examples}
        ...
        * model
        ...
        {cmd:. quiet clspl2 ap, b(rhs) model(M) m(`m') p(`p')}
        ...
        * model
        ...
        {cmd:. quiet clspl2 cmls, b(b) c(C) s(S) mod(D*) r(1)}
...
{pstd}
{cmd:clspl2} stores the following in {cmd:e()}:
...
{synopt:{cmd:e(alpha)}}regularization parameter (weight) in the final convex
       program of the CLSP estimator; for {cmd:clspl2} always {bf:1} (added for
       compatibility with the Python-based {helpb clsp}){p_end}
...
{synopt:{cmd:e(cmd)}}{cmd:clspl2}{p_end}
