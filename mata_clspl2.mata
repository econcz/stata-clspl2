									 ...
									 `RS' zero_diagonal)           /* public  */
{
	...
	*/
	`RS' M_flag, k, n_col
	`RM' C0, S0, M0, row_groups, col_groups, M_diag
	`TM' tmp, rc
	C0 = C; S0 = S; M0 = M                             //   store C, S, and M   
	i = i < . ? i : 1
	j = j < . ? j : 1
	...
	C = C0; S = S0; M = M0                             // restore C, S, and M   
}
								  ...
								  `RS' cond_tolerance)             /* public  */
{
	...
	*/
	...
	if (rows(this.A) > this.C_idx[1]) {
		M_part             = this.A[this.C_idx[1]+1::rows(this.A),
									1..this.C_idx[2]]
		b_M                = this.b[|rows(this.b)-rows(M_part)+1\.|]
		residuals_M        = b_M - M_part * colshape(this.x, 1)
