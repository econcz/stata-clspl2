...
program define clspl2, eclass byable(recall)
	...
	*/
	...
	[anything] [if] [in], [													///
		...
		CONDTOLerance(real 1e-14) nested x symmetric *						///
	]
	...
	if ("`nested'" !=  "" & "`x'"      !=  ""      )                          {
		if ("`symmetric'"              !=  ""                             & ///
		    rows(st_matrix("e(x)")) != cols(st_matrix("e(x)"))            )   {
			di as err  "symmetric option requires a square "                ///
					   "matrix (m == p)"
			error 198
		}
		mata: st_matrix("e(X)", "`symmetric'" == "" ?  st_matrix("e(x)")    ///
			           :  0.5 *  (st_matrix("e(x)") +  st_matrix("e(x)")' ));
	}
